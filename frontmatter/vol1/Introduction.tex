...
The answer runs through binary switching, number systems, integers, floating-point, characters, byte ordering, bitwise operations, memory alignment, pointers, error detection, media formats, and serialization. By the end, you have everything you need to understand what's actually happening when you write \texttt{int x = 42} or read a JPEG from disk.

The other two volumes pick up from here. Volume II covers the hardware caches, pipelines, GPUs. Volume III covers arrays themselves in full. This volume is the foundation both of them depend on.
